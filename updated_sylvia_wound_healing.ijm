...

{
   ...
   if (roiManager("count")>=1) {
      ...
      title_image=getInfo("slice.label");
      FileName = Array.concat(FileName,title_image);
      if (splice == "Yes") {
         splices = split(title_image,'_');
         for (i=0;i<splices.length;i++) {
            if (i == 0) {
              Split0 = Array.concat(Split0,splices[i]);
            }
            else if (i == 1) {
              Split1 = Array.concat(Split1,splices[i]);
            }
            else if (i == 2) {
              Split2 = Array.concat(Split2,splices[i]);
            }
            else if (i == 3) {
              Split3 = Array.concat(Split3,splices[i]);
            }
            else if (i == 4) {
              Split4 = Array.concat(Split4,splices[i]);
            }
            else if (i == 5) {
              Split5 = Array.concat(Split5,splices[i]);
            }
            else if (i == 6) {
              Split6 = Array.concat(Split6,splices[i]);
            }
            else if (i == 7) {
              Split7 = Array.concat(Split7,splices[i]);
            }
            else if (i == 8) {
              Split8 = Array.concat(Split8,splices[i]);
            }
         }
      }
   ...
   if (splice == "Yes") {
      if (Split4.length == 0) {
         Array.show("Results (row numbers)", FileName,
         Split0,Split1,Split2,Split3,
         Area,AreaFraction,Width,StandarDeviation);
      }
      if (Split5.length == 0) {
         Array.show("Results (row numbers)", FileName,
         ...
         Array.show("Results (row numbers)", FileName,
         Split0,Split1,Split2,Split3,Split4,Split5,Split6,Split7,Split8,
         Area,AreaFraction,Width,StandarDeviation);
      }
   }
   else {
      Array.show("Results (row numbers)", FileName,Area,AreaFraction,Width,StandarDeviation);
   }
   scale_fixed(scale);
   setTool("rectangle");
...

{
   ...
   title_image=getTitle();
   setResult("Label", n1, title_image);
   setResult("Area "+unit+"^2", n1, total_area);
   setResult("Area %", n1, area_fraction);
   setResult("Width "+unit, n1, avg_width);
   setResult("Standard deviation "+unit, n1, std_dist);
   scale_fixed(scale);
...
set_scale_global = newArray("Yes","No");
   ...
   Dialog.addChoice("Splice file names?", splice_files);
